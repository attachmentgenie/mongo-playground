...
create_cfg_srv() {
  ID=$(docker ps | grep cfg$@ |  awk '{print $1}')
  if [[ -z "$ID" ]]; then
...
create_mongos_srv() {
  ID=$(docker ps | grep mongos${@} |  awk '{print $1}')
  if [[ -z "$ID" ]]; then
...
create_rs_srv() {
  ID=$(docker ps | grep rs${1}_srv${2} |  awk '{print $1}')
  if [[ -z "$ID" ]]; then
    ...
    while [ $attempt -le 59 ]; do
      attempt=$(( $attempt + 1 ))
      echo "Waiting for server to be up (attempt: $attempt)..."
      result=$(docker logs rs${1}_srv${2})
      if grep -q 'waiting for connections on port 27017' <<< $result ; then
        echo "Mongodb is up!"
        break
      fi
...
rs.initiate(config);
EOF
    mongo --quiet --port $(docker-port rs${i}_srv1) < rs${i}.js
    attempt=0
    while [ $attempt -le 59 ]; do
      attempt=$(( $attempt + 1 ))
...
EOF
    mongo --quiet --port $(docker-port mongos1) < rs${i}_sh.js
  fi
done
